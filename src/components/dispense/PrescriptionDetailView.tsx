...
import { ArrowLeft, Ruler, Weight, Calendar, Building2, ClipboardList, Bot, CheckCircle2, MessageSquare } from 'lucide-react';
...
interface PrescriptionDetailViewProps {
  ...
  cardKey: CardKey;
  onBack: () => void;
  onComplete: (regimen: Regimen) => void;
}

export default function PrescriptionDetailView({ regimen, cardKey, onBack, onComplete }: PrescriptionDetailViewProps) {
        ...
        <div className="flex flex-col gap-3 flex-1 min-h-0">
          ...
          {cardKey === 'receivable' && (
            <div className="flex items-center justify-end glass-card px-5 py-3 flex-shrink-0">
              <button
                onClick={() => onComplete(regimen)}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-base font-semibold bg-amber-500 text-white shadow-[0_2px_16px_rgba(245,158,11,0.35)] hover:bg-amber-600 hover:shadow-[0_4px_22px_rgba(245,158,11,0.45)] active:scale-[0.97] transition-all duration-200 cursor-pointer"
              >
                確認簽收
              </button>
            </div>
