  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 w-28 flex-shrink-0">
        ...
        <span className="text-slate-600 text-base font-mono">{rx.effectiveDateTime.split(' ')[1]}</span>
      </div>

      <div className="flex items-center gap-2 flex-1 min-w-0">
        <User className="w-4 h-4 text-slate-400 flex-shrink-0" />
        <span className="text-slate-800 text-base font-semibold truncate">{rx.patientName}</span>
      </div>

      <div className="flex items-center gap-1.5 w-32 flex-shrink-0">
        <FileText className="w-4 h-4 text-slate-400" />
        <span className="text-slate-500 text-base font-mono">{rx.chartNumber}</span>
      </div>

      {patientType === 'inpatient' && rx.admissionNumber ? (
        <div className="flex items-center gap-1.5 w-40 flex-shrink-0">
          <Building2 className="w-4 h-4 text-amber-400" />
          <span className="text-amber-700 text-base font-mono truncate">{rx.admissionNumber}</span>
        </div>
      ) : (
        <div className="flex items-center gap-1.5 flex-1 min-w-0 flex-shrink-0">
          <Stethoscope className="w-4 h-4 text-slate-400" />
          <span className="text-slate-600 text-base truncate">{rx.department}</span>
        </div>
      )}

      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-semibold flex-shrink-0 ${capsule.className}`}>
        {capsule.label}
      </span>
